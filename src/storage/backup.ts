...
export function parseBackup(text: string): ParsedBackup {
  ...
  return { data, preview: { backupVersion, schemaVersion: data.schemaVersion, projects: data.projects.length, activities: data.activity.length, appVersion, exportedAt } };
}

export function serializeBackup(raw: string | null, appVersion: string, storageKey: string): string {
  const metadata = { app: 'Logic Core', backupVersion: BACKUP_VERSION, appVersion, exportedAt: new Date().toISOString(), storageKey };
  try { return JSON.stringify({ ...metadata, data: parseData(raw) }, null, 2); }
