...
export function DecisionsPage() {
  ...
  return <>
    <PageHeader eyebrow="STRATEGY / DECISIONS" title="Beslutninger" />
    <p className="decision-intro muted">Tænk klart. Vælg selv. Bevar det, du troede på.</p>
    <a className="button" href="#/decisions/new"><Icon name="plus" size={18} />Ny beslutning</a>
    <div className="decision-filters project-form">
    ...
    <div className="project-list">{visible.map(decision => {
      ...
      return <a className="project-row" key={decision.id} href={`#/decisions/${encodeURIComponent(decision.id)}`}>
        <span className={`project-marker status-${decision.status}`} />
        <div className="project-content"><div className="project-topline"><span className={`badge status-${decision.status}`}>{DECISION_LABELS[decision.status]}</span>{isReviewDue(decision, now) && <span className="badge review-due">Review forfalder</span>}</div>
