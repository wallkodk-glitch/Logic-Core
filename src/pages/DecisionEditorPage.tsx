...
export function DecisionEditorPage({ id }: { id: string }) {
  ...
  if (id !== 'new' && !decision) return <><PageHeader eyebrow="DECISIONS" title="Beslutningen findes ikke" /><p className="muted">Den kan være slettet eller erstattet via restore i et andet vindue.</p><a className="button secondary" href="#/decisions">Til beslutninger</a></>;
...
function DecisionEditor({ decision }: { decision: Decision | undefined }) {
  ...
  function transition(action: 'reopen' | 'close' | 'archive') {
    if (!original) return;
    if (dirty) { setNotice('Gem kladden før statusændring.'); return; }
  ...
  const saveButton = <button type="button" className="button" disabled={!input.title.trim() || stale} onClick={() => save()}>Gem kladde</button>;
  ...
  return <div className="decision-editor">
    ...
    <PageHeader eyebrow={original ? 'DECISION / WORKSPACE' : 'DECISION / CREATE'} title={original ? original.title : 'Ny beslutning'} />
    <div className="project-topline"><span className={`badge status-${original?.status ?? 'draft'}`}>{DECISION_LABELS[original?.status ?? 'draft']}</span>{original && isReviewDue(original, now) && <span className="badge review-due">Review forfalder</span>}</div>
    <p className="field-help">{dirty ? 'Ændringer er ikke gemt. Gem kladden løbende og før navigation/lukning.' : `Gemt lokalt${original ? ` · ${formatTime(original.updatedAt)}` : ''}.`}</p>
    ...
    <form className="project-form" onSubmit={event => { event.preventDefault(); save(); }}>
      {draft && <div className="form-actions">{saveButton}</div>}
      <fieldset className="decision-workspace" disabled={!draft || stale}>
        <details className="decision-section" open><summary>01 · Goal</summary>
        ...
        <details className="decision-section" open><summary>02 · Reality</summary>
        ...
        <details className="decision-section" open><summary>03 · Constraints + Assumptions</summary>
        ...
        <details className="decision-section" open><summary>04 · Options + Trade-offs</summary><DecisionOptions options={input.options} onChange={optionsChanged} /></details>
        <details className="decision-section" open><summary>05 · Criteria + Scoring · valgfri</summary><DecisionScoring content={input} onCriteria={criteriaChanged} onScores={scores => change({ ...input, scores })} /></details>
        <details className="decision-section" open><summary>06 · Decision</summary>
        ...
        <details className="decision-section" open><summary>07 · Execution + Review</summary>
      ...
      {draft && <><p className="field-help">“Beslut” gemmer hele formularen og et nyt, uændret historisk snapshot i samme handling.</p><div className="form-actions">{saveButton}<button type="button" className="button secondary" disabled={stale} onClick={() => save(true)}>Beslut og gem snapshot</button></div></>}
    </form>
    ...
    {original && <>
      ...
      <section className="decision-section"><h2>Beslutningens livscyklus</h2><p className="field-help">Arkivering og genåbning bevarer historik. Sletning fjerner beslutningen inklusive alle snapshots og reviews.</p>
