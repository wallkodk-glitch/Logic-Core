...
import { RelatedWork } from '../components/RelatedWork.tsx';
...
function ProjectEditor({ project }: { project: Project | undefined }) {
  ...
  const releaseWork = useUnsavedWork(title !== (original?.title ?? '') || description !== (original?.description ?? '') || status !== (original?.status ?? 'brainstorm'));
  function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const result = store.saveProject({ title, description, status }, original?.id, original?.updatedAt);
    if (result.ok) { releaseWork(); navigate('/projects'); } else setNotice(result.error);
  }
  function remove() {
    if (!original) return;
